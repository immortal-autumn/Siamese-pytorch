...
def main():
    ...
    detect_and_return(args['input_image'])
...
def detect_and_return(var):
    label, name, input_image = var
    cropped = None
    io.imread(input_image)
    d = Detector(input_image)
    d.detect()
    # if d.result.face_count != 1:
    #     return None
    # for i, face in enumerate(d.result.faces):
    #     cropped = Image.open(input_image)
    #     cropped = cropped.crop((face.left(),
    ...
    #                             face.right(),
    #                             face.bottom()))
    return d.result.__dict__, label, name
